...
import { DGGetUserId, DGIsRegisteredUser, DGLoadLevels, LevelsResult } from "../contract/DailyGame.ts";
import Moralis from "moralis";
import { useMoralis } from "react-moralis";
import { convertStringToNumber, greaterThanOrEqual } from "../helpers/bignumber.ts";
...
export const userIdSelector = selector<string | null>({
    ...
    get: async ({ get }) => {
        ...
        return await DGGetUserId(userAddress);
    },
});
...
export const userLevelsSelector = selector<LevelsResult | null>({
    ...
    get: async ({ get }) => {
        ...
        return await DGLoadLevels(userAddress);
    },
});
...
export default function AccountStore() {
    ...
    useEffect(() => {
        if (!isRegistered && userAddress) {
            DGIsRegisteredUser(userAddress).then(setRegistered);
        }
